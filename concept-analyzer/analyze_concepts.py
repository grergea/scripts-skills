...
def normalize_stem(s):
    """파일명 정규화 — 구분자(공백·하이픈·언더스코어) 제거 후 소문자."""
    return re.sub(r"[\s\-_]", "", s).lower()


def build_normalized_index(stems):
    """normalize(stem) → 원본 stem 매핑.

    여러 파일명이 같은 키로 정규화될 때(예: 'Exclusive IP' / 'Exclusive-IP')
    set 순회에 맡기면 어느 쪽이 '정답'으로 제안되는지가 실행마다 뒤바뀐다.
    CLAUDE.md 파일명 규칙(PascalCase-Hyphen, 공백·언더스코어 불가)을 지키는
    쪽을 우선하고, 그래도 동률이면 사전순으로 고정한다.
    """

    def rank(stem):
        return (1 if re.search(r"[\s_]", stem) else 0, stem)

    index = {}
    for stem in sorted(stems, key=rank):
        index.setdefault(normalize_stem(stem), stem)
    return index
...
class ConceptAnalyzer:
    ...
    def _build_vault_file_index(self):
        ...
        print(
            f"Built vault file index: {len(self.vault_file_stems)} files",
            file=sys.stderr,
        )
    ...
    def generate_statistics(self, additional_known_stems=None):
        ...
        mermaid_rate = (
            (mermaid_count / total_concepts * 100) if total_concepts > 0 else 0
        )

        # 평균 단어 수 및 섹션 수
        avg_words = (
            sum(c["word_count"] for c in self.concepts.values()) / total_concepts
            if total_concepts > 0
            else 0
        )
        avg_sections = (
            sum(c["section_count"] for c in self.concepts.values()) / total_concepts
            if total_concepts > 0
            else 0
        )
        ...
        normalized_to_stem = build_normalized_index(known_stems)
        ...
        for filename, concept in self.concepts.items():
            for link in concept["outlinks"]:
                ...
                if norm_link in normalized_to_stem:
                    ...
                    naming_mismatches[link].append(
                        {"source": filename, "correct": correct}
                    )
                elif " " not in link:
...
def print_summary(report, crosslink=None):
    ...
    for concept in stats["isolated_concepts"][:10]:
        print(
            f"   - {concept['filename']}: In={concept['inlinks_count']}, Out={concept['outlinks_count']}"
        )

    print("\n🌟 허브 개념 (Top 10):")
    for concept in stats["hub_concepts"][:10]:
        print(
            f"   - {concept['filename']}: In={concept['inlinks_count']}, Out={concept['outlinks_count']}, Total={concept['total_links']}"
        )
    ...
    if stats.get("dangling_links"):
        ...
        for item in stats["dangling_links"]:
            ...
            more = (
                f" 외 {len(item['referenced_by']) - 3}개"
    ...
    if stats.get("naming_mismatches"):
        ...
        for item in stats["naming_mismatches"]:
            ...
            print(
                f"   - [[{item['link']}]] → [[{item['correct']}]] (참조: {refs}{more})"
            )

    if crosslink:
        print("\n🔗 카테고리 간 크로스링크")
        print(
            f"   {crosslink['path_a']} → {crosslink['path_b']}: {crosslink['a_to_b_total']}개"
        )
        if crosslink["a_to_b_top"]:
            for stem, count in crosslink["a_to_b_top"][:5]:
                print(f"      └ {stem}: {count}회")
        print(
            f"   {crosslink['path_b']} → {crosslink['path_a']}: {crosslink['b_to_a_total']}개"
        )
...
class ConceptMiner:
    ...
    PERSONAL_DIRS = {"팀관리_면담", "개인_성장", "개인_금융", "개인_홈", "개인_여행"}
    ...
    def build_indexes(self, concepts_tech_path, concepts_personal_path):
        ...
        self.concept_stems_normalized = build_normalized_index(self.concept_stems)
        print(
            f"Index: 볼트 {len(self.vault_file_stems)}개 파일, Concept {len(self.concept_stems)}개",
            file=sys.stderr,
        )
    ...
    def _find_duplicate(self, candidate):
        ...
        return self.concept_stems_normalized.get(normalize_stem(candidate))
    ...
    def extract_keyword_candidates(self, body):
        ...
        hyphen = re.findall(
            r"\b[A-Z][a-zA-Z]+-[A-Z][a-zA-Z]+(?:-[A-Z][a-zA-Z]+)?\b", clean
        )

        return [
            kw for kw in pascal + allcaps + hyphen if kw not in self.KEYWORD_BLOCKLIST
        ]
    ...
    def mine(self, concepts_tech_path, concepts_personal_path):
        ...
        print(
            f"스캔 대상: {len(target_files)}개 파일 (days={self.days})", file=sys.stderr
        )

        wikilink_candidates = defaultdict(
            lambda: {"sources": [], "category": "Tech", "method": "wikilink"}
        )
...
def print_mining_report(result):
    ...
    print(
        f"기존 Concept 수: {result['existing_concepts']}개 | 신규 후보: {len(candidates)}개"
    )
    print("=" * 60)
...
def create_concept_notes(
    candidates, vault_root, concepts_tech_path, concepts_personal_path
):
    ...
    for name, info in candidates.items():
        ...
        target_dir = (
            Path(concepts_tech_path)
            if category == "Tech"
            else Path(concepts_personal_path)
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze Obsidian Concept notes network"
    )
    parser.add_argument(
        "--mode",
        choices=["analyze", "mine"],
        default="analyze",
    ...
    parser.add_argument(
        "--crosslink-path",
        help="Second Concepts folder for cross-link analysis (optional)",
    )
    ...
    parser.add_argument(
        "--tech-path",
        help="Concepts_Tech 경로 (기본값: vault/03_Resources/Concepts_Tech)",
    )
    parser.add_argument(
        "--personal-path",
        help="Concepts_Personal 경로 (기본값: vault/03_Resources/Concepts_Personal)",
    )
    parser.add_argument(
        "--days",
        type=int,
    ...
    if args.mode == "mine":
        ...
        personal_path = args.personal_path or str(
